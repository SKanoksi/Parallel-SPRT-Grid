...
PSPS_OpenGL_FIM_3D_Solver::PSPS_OpenGL_FIM_3D_Solver()
{
...
bool PSPS_OpenGL_FIM_3D_Solver::Init()
{
   ...
   side = 1 ;
   front = length[0] ;
   top = length[0]*length[1] ;

   // Read code
   std::string comCode, temp ;
   if( !readSource("./PSPS_FIM_Solver3D.comSh",comCode) ){ // The code must be found ###
      std::cout << "ERROR:: PSPS_FIM_Solver3D.comSh is not found (in the same folder as this program).\n" ;
      return false;
   }
   ...
   comCode.replace( comCode.find("SHARED_Y"), 8, temp.c_str() );
     ToString( block[2]+2,temp);
   comCode.replace( comCode.find("SHARED_Z"), 8, temp.c_str() );
     ToString( block[0],temp);
   comCode.replace( comCode.find("BLock_X"), 7,  temp.c_str() );
     ToString( block[1],temp);
   comCode.replace( comCode.find("BLock_Y"), 7, temp.c_str() );
     ToString( block[2],temp);
   comCode.replace( comCode.find("BLock_Z"), 7, temp.c_str() );
     ToString( block[0]+block[1]+block[2], temp);
...
bool PSPS_OpenGL_FIM_3D_Solver::Set()
{
...
bool PSPS_OpenGL_FIM_3D_Solver::Compute()
{
    /// INITIAL
   // Open program
...
bool PSPS_OpenGL_FIM_3D_Solver::Retrieve()
{
   /// Retrieve Maps ///
   // Traveltime x1
...
bool PSPS_OpenGL_FIM_3D_Solver::Clear()
{
...
void PSPS_OpenGL_FIM_3D_Solver::addDummyVertices()
{
   GLuint at ;
...
void PSPS_OpenGL_FIM_3D_Solver::removeDummyVertices()
{
   GLuint at ;
   std::vector<TyPe> temp ;
