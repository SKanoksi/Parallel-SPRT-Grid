...
PSPS_OpenGL_FIM_3D_Tools::PSPS_OpenGL_FIM_3D_Tools()
{
...
bool PSPS_OpenGL_FIM_3D_Tools::newProject(std::string &inFile, std::vector<int> &param)
{
   // Read slowness model.
...
bool PSPS_OpenGL_FIM_3D_Tools::resumeProject(std::string &inFile, int maxloop)
{
...
bool PSPS_OpenGL_FIM_3D_Tools::saveProject(std::string &inFile)
{
   ...
   if( remove(inFile.c_str())!=0 ){
      ...
      inFile = inFile + "_PSPS_OpenGL_FIM.out" ;
   }
...
bool PSPS_OpenGL_FIM_3D_Tools::readSlownessModel(const char* filePath) // Read res and slowness.
{
	// NetCDF error handling
	#ifndef nCDF
	  #define nCDF(Func) { int NC_Stat; if( NC_NOERR != (NC_Stat=Func) ){ fprintf(stderr, "ERROR::NETCDF:: %s\n", nc_strerror(NC_Stat)); std::exit(2);} }
...
bool PSPS_OpenGL_FIM_3D_Tools::inputSpec(std::vector<int> &param)
{
...
bool PSPS_OpenGL_FIM_3D_Tools::readResumeProject(const char* filePath) // Read all maps and Setup.
{
    bool flag = true ;
...
bool PSPS_OpenGL_FIM_3D_Tools::checkSpec()
{
   bool flag = true ;

   /// Get Computer Spec (OpenGL_FIM)
   ...
   for(GLuint i=0 ; i<DIM ; ++i)
   {
      if( block[i] > (GLuint)l[i] ){
         std::cout << "CheckSpec >> Cannot run on this Computer, a \"Block\" is larger than GL_MAX_COMPUTE_WORK_GROUP_SIZE !!!\n" ;
         flag = false ;         
         break; 
      }
   ...
   if( WorkGroup_Size > (GLuint)nInvo ){
      std::cout << "CheckSpec >> Invalid \"Block\" value, bigger than GL_MAX_COMPUTE_WORK_GROUP_INVOCATIONS !!!\n" ;
      return false ;
   }
...
bool PSPS_OpenGL_FIM_3D_Tools::rescaleSlowness(std::vector<GLuint> &res_old)
{
