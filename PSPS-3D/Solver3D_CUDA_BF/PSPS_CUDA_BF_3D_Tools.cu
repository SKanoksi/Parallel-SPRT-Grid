...
#include "PSPS_CUDA_BF_3D_Tools.h"
...
PSPS_CUDA_BF_3D_Tools::PSPS_CUDA_BF_3D_Tools()
{
...
bool PSPS_CUDA_BF_3D_Tools::newProject(std::string &inFile, std::vector<int> &param)
{
   // Read slowness model.
...
bool PSPS_CUDA_BF_3D_Tools::resumeProject(std::string &inFile, int maxloop)
{
...
bool PSPS_CUDA_BF_3D_Tools::saveProject(std::string &inFile)
{
...
bool PSPS_CUDA_BF_3D_Tools::readSlownessModel(const char* filePath) // Read res and slowness.
{
	// NetCDF error handling
	#ifndef nCDF
	  #define nCDF(Func) { int NC_Stat; if( NC_NOERR != (NC_Stat=Func) ){ fprintf(stderr, "ERROR::NETCDF:: %s\n", nc_strerror(NC_Stat)); std::exit(2);} }
...
bool PSPS_CUDA_BF_3D_Tools::inputSpec(std::vector<int> &param)
{
...
bool PSPS_CUDA_BF_3D_Tools::readResumeProject(const char* filePath) // Read all maps and Setup.
{
    bool flag = true ;
...
bool PSPS_CUDA_BF_3D_Tools::checkSpec()
{
   bool flag = true ;
...
bool PSPS_CUDA_BF_3D_Tools::rescaleSlowness(std::vector<unsigned int> &res_old)
{
   ...
   cudaBindSurfaceToArray(outMap, cuArrayOut, channelDesc);
