...
class PSPS_CUDA_ExtendedBF_3D_Program
{
public:
   explicit PSPS_CUDA_ExtendedBF_3D_Program();
   ~PSPS_CUDA_ExtendedBF_3D_Program();

   void runResumeProject(std::string &inFile, int maxloop);
   void runNewProject(std::string &inFile, std::vector<int> &param);

private:
   PSPS_CUDA_ExtendedBF_3D_Solver* solver ;
   PSPS_CUDA_ExtendedBF_3D_Tools* asis ;
