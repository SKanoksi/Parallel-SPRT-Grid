...
#include "PSPS_OpenGL_ExtendedBF_3D_Tools.h"

#define TyPe GLfloat
#define NC_TyPe NC_FLOAT
#define DIM 3
#define NAME_DATA "Slowness"
#define GPU_MEMORY_LIMIT 0.98     // Will use at most 90% of total GPU memory. ###
#define SHARED_MEMORY_LIMIT 0.98  // Will use at most 98% of total GPU shared memory. ###

PSPS_OpenGL_ExtendedBF_3D_Tools::PSPS_OpenGL_ExtendedBF_3D_Tools()
{
...
bool PSPS_OpenGL_ExtendedBF_3D_Tools::newProject(std::string &inFile, std::vector<int> &param)
{
   // Read slowness model.
...
bool PSPS_OpenGL_ExtendedBF_3D_Tools::resumeProject(std::string &inFile, int maxloop)
{
...
bool PSPS_OpenGL_ExtendedBF_3D_Tools::saveProject(std::string &inFile)
{
   ...
   if( remove(inFile.c_str())!=0 ){
      ...
      inFile = inFile + "_PSPS_OpenGL_ExtendedBF.out" ;
   }
...
bool PSPS_OpenGL_ExtendedBF_3D_Tools::readSlownessModel(const char* filePath) // Read res and slowness.
{
	// NetCDF error handling
	#ifndef nCDF
	  #define nCDF(Func) { int NC_Stat; if( NC_NOERR != (NC_Stat=Func) ){ fprintf(stderr, "ERROR::NETCDF:: %s\n", nc_strerror(NC_Stat)); std::exit(2);} }
...
bool PSPS_OpenGL_ExtendedBF_3D_Tools::inputSpec(std::vector<int> &param)
{
   block[0]  = std::abs( 2*param[0] ) ;
   block[1]  = std::abs( 2*param[1] ) ;
   block[2]  = std::abs( 2*param[2] ) ;
...
bool PSPS_OpenGL_ExtendedBF_3D_Tools::readResumeProject(const char* filePath) // Read all maps and Setup.
{
    bool flag = true ;
...
bool PSPS_OpenGL_ExtendedBF_3D_Tools::checkSpec()
{
   bool flag = true ;

   /// Get Computer Spec (OpenGL_ExtendedBF)
   ...
   // Only for 3D ###
   GLuint needMem = 0 ;
   needMem += 3*sizeof(TyPe)*(res[0]+block[0])*(res[1]+block[1])*(res[2]+block[2]) ; // ( Slowness+Traveltime x2 )*( res+dummy)
   needMem += sizeof(GLint)*res[0]*res[1]*res[2] ; // Raypath
   needMem += 2*sizeof(GLint)*(nWorkGroup[0]+2)*(nWorkGroup[1]+2)*(nWorkGroup[2]+2) ; // Update map x2 (but will use only 0,1)
...
bool PSPS_OpenGL_ExtendedBF_3D_Tools::rescaleSlowness(std::vector<GLuint> &res_old)
{
   ...
   if( !readSource("./PSPS_Rescale3D.comSh",comCode) ){ // The code must be found ###
       std::cout << "ERROR:: PSPS_Rescale3D.comSh is not found (in the same folder as this program).\n" ;
       return false;
   }
   ...
   comCode.replace( comCode.find("RES_Y"), 5, temp.c_str() );
     ToString(res[2],temp); temp += ".0f" ;
   comCode.replace( comCode.find("RES_Z"), 5, temp.c_str() );
   // Compile code
   ...
   // Set input texture.
   glBindTexture(GL_TEXTURE_3D, inTexture);
      // Input image.
      glTexImage3D(GL_TEXTURE_3D, 0, GL_R32F, res_old[0], res_old[1], res_old[2], 0, GL_RED, GL_FLOAT, (GLvoid*)&slowness[0]);
      // Set texture.
   ...
   glBindTexture(GL_TEXTURE_3D, outTexture);
      glTexImage3D(GL_TEXTURE_3D, 0, GL_R32F, res[0], res[1], res[2], 0, GL_RED, GL_FLOAT, NULL);
   ...
   glUseProgram(rescaleComSh);
      ...
      glBindImageTexture(0, outTexture, 0, GL_TRUE, 0, GL_WRITE_ONLY, GL_R32F); // For 3D_TEXTURE, isLayered must be GL_TRUE !!!
         glDispatchCompute( nWorkGroup[0], nWorkGroup[1], nWorkGroup[2]);
   ...
   glBindTexture(GL_TEXTURE_3D, outTexture);
     ...
     glGetTexImage(GL_TEXTURE_3D, 0, GL_RED, GL_FLOAT, &slowness[0]);
   glBindTexture(GL_TEXTURE_3D, 0);
