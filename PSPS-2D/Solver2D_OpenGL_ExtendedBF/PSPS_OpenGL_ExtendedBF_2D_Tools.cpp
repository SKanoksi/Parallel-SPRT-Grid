...
PSPS_OpenGL_ExtendedBF_2D_Tools::PSPS_OpenGL_ExtendedBF_2D_Tools()
{
...
bool PSPS_OpenGL_ExtendedBF_2D_Tools::newProject(std::string &inFile, std::vector<int> &param)
{
   // Read slowness model.
...
bool PSPS_OpenGL_ExtendedBF_2D_Tools::resumeProject(std::string &inFile, int maxloop)
{
...
bool PSPS_OpenGL_ExtendedBF_2D_Tools::saveProject(std::string &inFile)
{
...
bool PSPS_OpenGL_ExtendedBF_2D_Tools::readSlownessModel(const char* filePath) // Read res and slowness.
{
	// NetCDF error handling
	#ifndef nCDF
	  #define nCDF(Func) { int NC_Stat; if( NC_NOERR != (NC_Stat=Func) ){ fprintf(stderr, "ERROR::NETCDF:: %s\n", nc_strerror(NC_Stat)); std::exit(2);} }
...
bool PSPS_OpenGL_ExtendedBF_2D_Tools::inputSpec(std::vector<int> &param)
{
   block[0]  = std::abs( 4 ) ;
   block[1]  = std::abs( 4 ) ;
   res[0]    = std::abs( param[0] ) ;
   res[1]    = std::abs( param[1] ) ;
   source[0] = std::abs( param[2] ) ;
   source[1] = std::abs( param[3] ) ;
   maxLoop   = std::abs( param[4] ) ;
...
bool PSPS_OpenGL_ExtendedBF_2D_Tools::readResumeProject(const char* filePath) // Read all maps and Setup.
{
    bool flag = true ;
   ...
   for(GLuint i=0 ; i<DIM ; ++i)
   {
      ...
      // Check and Compute nWorkGroup
      if( res[i]%block[i] != 0 ){
        std::cout << "ResumeProject >> A number of cells is invalid, it MUST be divided by 4 or radius==2 !!!\n" ;
        flag = false ;         
        break;
      }else{
         nWorkGroup[i] = (GLuint) res[i]/block[i] ;
...
bool PSPS_OpenGL_ExtendedBF_2D_Tools::checkSpec()
{
   bool flag = true ;
...
bool PSPS_OpenGL_ExtendedBF_2D_Tools::rescaleSlowness(std::vector<GLuint> &res_old)
{
