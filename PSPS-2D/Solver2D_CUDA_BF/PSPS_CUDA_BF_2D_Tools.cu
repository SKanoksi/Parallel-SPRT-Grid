...
PSPS_CUDA_BF_2D_Tools::PSPS_CUDA_BF_2D_Tools()
{
...
bool PSPS_CUDA_BF_2D_Tools::newProject(std::string &inFile, std::vector<int> &param)
{
   // Read slowness model.
...
bool PSPS_CUDA_BF_2D_Tools::resumeProject(std::string &inFile, int maxloop)
{
...
bool PSPS_CUDA_BF_2D_Tools::saveProject(std::string &inFile)
{
   ...
   if( remove(inFile.c_str())!=0 ){
      ...
      inFile = inFile + "_PSPS_CUDA_BF.out" ;
   }
...
bool PSPS_CUDA_BF_2D_Tools::readSlownessModel(const char* filePath) // Read res and slowness.
{
	// NetCDF error handling
	#ifndef nCDF
	  #define nCDF(Func) { int NC_Stat; if( NC_NOERR != (NC_Stat=Func) ){ fprintf(stderr, "ERROR::NETCDF:: %s\n", nc_strerror(NC_Stat)); std::exit(2);} }
...
bool PSPS_CUDA_BF_2D_Tools::inputSpec(std::vector<int> &param)
{
...
bool PSPS_CUDA_BF_2D_Tools::readResumeProject(const char* filePath) // Read all maps and Setup.
{
...
bool PSPS_CUDA_BF_2D_Tools::checkSpec()
{
   bool flag = true ;
...
bool PSPS_CUDA_BF_2D_Tools::rescaleSlowness(std::vector<unsigned int> &res_old)
{
