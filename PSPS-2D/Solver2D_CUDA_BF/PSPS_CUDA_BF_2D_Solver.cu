...
PSPS_CUDA_BF_2D_Solver::PSPS_CUDA_BF_2D_Solver()
{
...
__device__ float findWeight(const float *Slowness, const int ix, const int iy, const int2 Source) // Vertex = relative directional vector wrt. source(posOnShmem)
{
    ...
    Weight *= sqrt( powf(__int2float_rn(ix)*scale_X,2) + powf(__int2float_rn(iy)*scale_Y,2) )  ;
...
__device__ void compareTraveltime(const float *Traveltime, const float *Slowness, bool *isUpdated)
{
	...
	for(int j = -radius_Y ; j<0 ; ++j )
	{
		...
		{
			float newTt = Traveltime[ (posWorker.y+j)*shared_X + posWorker.x+i ]  + findWeight(Slowness, i,j , posWorker) ;
			if( newTt<Tt ){
				Tt = newTt ;
...
bool PSPS_CUDA_BF_2D_Solver::Compute()
{
   ...
   { ///***** =.\= Parallel Shortest Path Solver (CUDA_BF) =.\= *****///

      // Current Time
      time_t rawtime ;
      
      // Start
      std::cout << "\nPSPS_CUDA_BF >> Start Bellman-Ford (CUDA_BF).\n" ;
      time(&rawtime);
      ...
      std::cout << "PSPS_CUDA_BF >> Finish Bellman-Ford (CUDA_BF).\n\n" ;
...
void PSPS_CUDA_BF_2D_Solver::addDummyVertices()
{
   unsigned int at ;
...
void PSPS_CUDA_BF_2D_Solver::removeDummyVertices()
{
   unsigned int at ;
   std::vector<TyPe> temp ;
