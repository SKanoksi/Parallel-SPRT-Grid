...
#include "PSPS_CUDA_ExtendedBF_2D_Tools.h"
...
PSPS_CUDA_ExtendedBF_2D_Tools::PSPS_CUDA_ExtendedBF_2D_Tools()
{
...
bool PSPS_CUDA_ExtendedBF_2D_Tools::newProject(std::string &inFile, std::vector<int> &param)
{
   // Read slowness model.
   ...
   {
        std::cout << "\n!!! ERROR::ExtendedPSPS:: Difference of grid specing (hx,hy) is more than 1 percent. !!!\n\n" ;
        return false;
   }
...
bool PSPS_CUDA_ExtendedBF_2D_Tools::resumeProject(std::string &inFile, int maxloop)
{
...
bool PSPS_CUDA_ExtendedBF_2D_Tools::saveProject(std::string &inFile)
{
   ...
   if( remove(inFile.c_str())!=0 ){
      ...
      inFile = inFile + "_PSPS_CUDA_ExtendedBF.out" ;
   }
...
bool PSPS_CUDA_ExtendedBF_2D_Tools::readSlownessModel(const char* filePath) // Read res and slowness.
{
	// NetCDF error handling
	#ifndef nCDF
	  #define nCDF(Func) { int NC_Stat; if( NC_NOERR != (NC_Stat=Func) ){ fprintf(stderr, "ERROR::NETCDF:: %s\n", nc_strerror(NC_Stat)); std::exit(2);} }
...
bool PSPS_CUDA_ExtendedBF_2D_Tools::inputSpec(std::vector<int> &param)
{
   ...
   for(int i=0 ; i<DIM ; ++i)
   {
      ...
      if( res[i]%block[i] != 0 ){
          std::cout << "NewProject >> A number of cells is invalid, it MUST be divisible by 4 !!!\n" ;
          return false ;
      }else{
...
bool PSPS_CUDA_ExtendedBF_2D_Tools::readResumeProject(const char* filePath) // Read all maps and Setup.
{
   std::cout << "ResumeProject >> Reading the project (NetCDF).\n" ;
   bool flag = true ;
   ...
   if( inputState[0] != 0 ){
      std::cout << "ResumeProject >> This project is marked as \"Finish\".\n"
                << "Exit PSPS_CUDA_ExtendedBF, Thank you.\n" ;
      return false;
   }else{
      isFinish = false ;
   }
   ...
   for(unsigned int i=0 ; i<DIM ; ++i)
   {
      ...
      // Check and Compute nWorkGroup
      if( res[i]%block[i] != 0 ){
         std::cout << "ResumeProject >> A resolution value is invalid, resolution MUST be divided by 4 (Radius = 2) !!!\n" ;
        flag = false ;         
        break;
      }else{
         nWorkGroup[i] = (unsigned int) res[i]/block[i] ;
...
bool PSPS_CUDA_ExtendedBF_2D_Tools::checkSpec()
{
   bool flag = true ;
...
texture<float, cudaTextureType2D, cudaReadModeElementType>  inMap ;
surface<void, cudaSurfaceType2D> outMap ;
...
__global__ void cudaRescaleSlowness()
{
	...
	if( x<resX && y<resY )
	{
		float data = tex2D(inMap,u,v);
		surf2Dwrite(data, outMap, sizeof(float)*x, y);
	}
	
}


bool PSPS_CUDA_ExtendedBF_2D_Tools::rescaleSlowness(std::vector<unsigned int> &res_old)
{
   ...
   cudaArray* cuArrayIn ;
   cudaMallocArray(&cuArrayIn, &channelDesc, res_old[0], res_old[1]);
   cudaMemcpyToArray(cuArrayIn, 0, 0, &slowness[0], res_old[0]*res_old[1]*sizeof(float), cudaMemcpyHostToDevice);
   ...
   cudaMallocArray(&cuArrayOut, &channelDesc, res[0], res[1], cudaArraySurfaceLoadStore);
   ...
   cudaMemcpyFromArray(&slowness[0], cuArrayOut, 0, 0, res[0]*res[1]*sizeof(float), cudaMemcpyDeviceToHost);
