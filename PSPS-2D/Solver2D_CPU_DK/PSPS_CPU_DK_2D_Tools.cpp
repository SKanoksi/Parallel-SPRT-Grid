...
#include "PSPS_CPU_DK_2D_Tools.h"
...
PSPS_CPU_DK_2D_Tools::PSPS_CPU_DK_2D_Tools()
{
...
bool PSPS_CPU_DK_2D_Tools::newProject(std::string &inFile, std::vector<int> &param)
{
   // Read slowness model.
...
bool PSPS_CPU_DK_2D_Tools::saveProject(std::string &inFile)
{
   ...
   if( remove(inFile.c_str())!=0 ){
      ...
      inFile = inFile + "_PSPS_CPU_DK.out" ;
   }
...
bool PSPS_CPU_DK_2D_Tools::readSlownessModel(const char* filePath) // Read res and slowness.
{
	// NetCDF error handling
	#ifndef nCDF
	  #define nCDF(Func) { int NC_Stat; if( NC_NOERR != (NC_Stat=Func) ){ fprintf(stderr, "ERROR::NETCDF:: %s\n", nc_strerror(NC_Stat)); std::exit(2);} }
...
bool PSPS_CPU_DK_2D_Tools::inputSpec(std::vector<int> &param)
{
...
bool PSPS_CPU_DK_2D_Tools::checkSpec()
{
   bool flag = true ;
