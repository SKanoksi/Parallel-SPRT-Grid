...
#include "PSPS_OpenGL_BF_2D_Solver.h"

PSPS_OpenGL_BF_2D_Solver::PSPS_OpenGL_BF_2D_Solver()
{
...
bool PSPS_OpenGL_BF_2D_Solver::Init()
{
   ...
   if( !readSource("./PSPS_BF_Solver2D.comSh",comCode) ){ // The code must be found ###
      std::cout << "ERROR:: PSPS_BF_Solver2D.comSh is not found.\n" ;
      return false;
   }
...
bool PSPS_OpenGL_BF_2D_Solver::Set()
{
   ...
   glBindTexture(GL_TEXTURE_2D, Raypath_Map);
     glTexImage2D(GL_TEXTURE_2D, 0, GL_R32I, res[0], res[1], 0, GL_RED_INTEGER, GL_INT, (GLvoid*)&raypath[0]);
      // Needed by Nvidia, even if not used. !!!     
     glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
     glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
...
bool PSPS_OpenGL_BF_2D_Solver::Compute()
{
   /// INITIAL
   // Open program
   ...
   glBindImageTexture(5, Raypath_Map  , 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_R32I);
   ...
   { ///***** =.\= Parallel Shortest Path Solver (OpenGL_BF Compute Shader) =.\= *****///
    
      // Current Time
      time_t rawtime ;
      
      // Start
      std::cout << "PSPS_OpenGL_BF >> Start Bellman-Ford (Compute shader).\n" ;
      time(&rawtime);
      ...
      std::cout << "PSPS_OpenGL_BF >> Finish Bellman-Ford (Compute shader).\n\n" ;
...
bool PSPS_OpenGL_BF_2D_Solver::Retrieve()
{
   /// Retrieve Maps ///
   // Traveltime x1
   ...
   // Raypath
   glBindTexture(GL_TEXTURE_2D, Raypath_Map);
     glGetTexImage(GL_TEXTURE_2D, 0, GL_RED_INTEGER, GL_INT, &raypath[0]);
...
bool PSPS_OpenGL_BF_2D_Solver::Clear()
{
...
void PSPS_OpenGL_BF_2D_Solver::addDummyVertices()
{
   GLuint at ;
...
void PSPS_OpenGL_BF_2D_Solver::removeDummyVertices()
{
   GLuint at ;
   std::vector<TyPe> temp ;
