...
#include "PSPS_CUDA_FIM_2D_Tools.h"
...
PSPS_CUDA_FIM_2D_Tools::PSPS_CUDA_FIM_2D_Tools()
{
...
bool PSPS_CUDA_FIM_2D_Tools::newProject(std::string &inFile, std::vector<int> &param)
{
   // Read slowness model.
   ...
   if( std::fabs(stride[0]-stride[1]) > 0.01*stride[1] ) // Only for 2D
   {
        std::cout << "\n!!! ERROR::FIM:: Difference of grid specing (hx,hy) is more than 1 percent. !!!\n\n" ;
        return false;
   }
...
bool PSPS_CUDA_FIM_2D_Tools::resumeProject(std::string &inFile, int maxloop)
{
...
bool PSPS_CUDA_FIM_2D_Tools::saveProject(std::string &inFile)
{
...
bool PSPS_CUDA_FIM_2D_Tools::readSlownessModel(const char* filePath) // Read res and slowness.
{
	// NetCDF error handling
	#ifndef nCDF
	  #define nCDF(Func) { int NC_Stat; if( NC_NOERR != (NC_Stat=Func) ){ fprintf(stderr, "ERROR::NETCDF:: %s\n", nc_strerror(NC_Stat)); std::exit(2);} }
...
bool PSPS_CUDA_FIM_2D_Tools::inputSpec(std::vector<int> &param)
{
...
bool PSPS_CUDA_FIM_2D_Tools::readResumeProject(const char* filePath) // Read all maps and Setup.
{
   ...
   for(unsigned int i=0 ; i<DIM ; ++i)
   {
      ...
      // Check and Compute nWorkGroup
      if( res[i]%block[i] != 0 ){
         std::cout << "ResumeProject >> A resolution value is invalid, resolution MUST be divided by 2*radius !!!\n" ;
        flag = false ;         
        break;
      }else{
         nWorkGroup[i] = (unsigned int) res[i]/block[i] ;
...
bool PSPS_CUDA_FIM_2D_Tools::checkSpec()
{
   bool flag = true ;
...
bool PSPS_CUDA_FIM_2D_Tools::rescaleSlowness(std::vector<unsigned int> &res_old)
{
